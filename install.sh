...
install_marzban() {
    ...
    if [ -f "$ENV_FILE" ]; then
        read -p ".env file already exists. Do you want to replace it? (Y/N) [default: N]: "
        if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    ...
    # Fetch and update Docker Compose file
    ...
    colorized_echo green "Marzban's files downloaded and setup successfully"
}
...
install_command() {
    ...
    check_version_exists() {
        local version=$1
        repo_url="https://api.github.com/repos/im-api/Marzban/releases"
